    ...
    module Helpers
      ...
      def authorized?
        if session[:auth_token] && Time.now.to_i < session[:auth_timeout] then
          # extend for another x amount of seconds
          session[:auth_timeout] = (Time.now+(timeout_seconds)).to_i
          return true 
        end
      ...
      def authenticate(klass, login, password)
        ...
        if u != nil then
          if u.attributes.key?(:lastlogin_at) then
            begin
              u.update(:lastlogin_at => DateTime.now)
            rescue NoMethodError => ex
              raise "You probably called DataMapper.finalize before you required the models."
            end
          end

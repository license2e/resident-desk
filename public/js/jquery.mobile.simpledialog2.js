...
(function($, undefined ) {
  $.widget( "mobile.simpledialog2", $.mobile.widget, {
	...
	_create: function () {
		...
		if ( o.dialogId != null ) { content.attr("id", o.dialogId); }	
	...
	_makeButtons: function () {
		...
		$.each(o.buttons, function(name, props) {
			...
			self.butObj.push($("<a href='#'>"+name+"</a>")
				...
				.attr('id', props.id)
				.addClass(cls)
				.buttonMarkup(buttonMarkup).unbind("vclick click")
	...
	_getCoords: function(widget) {
		var self = widget,
			...
			coords        = {
				...
				'winTop'  : ((widget.options.top.indexOf("%") != -1) ? widget.options.top : docWinHighOff + ((widget.options.top !== false ) ? widget.options.top : ( ( docWinHigh / 2 ) - ( diaWinHigh / 2 ) ))),
				'winLeft' : ((widget.options.left !== false && widget.options.left.indexOf("%") != -1) ? widget.options.left : (( docWinWidth / 2 ) - ( diaWinWidth / 2 ) ))
			};
	...
	_orientChange: function(e) {
		...
		if ( self.isDialog === true ) {
			...
			if ( o.fullScreen === true && ( coords.width < 400 || o.fullScreenForce === true ) ) {
				self.sdIntContent.css({'border': 'none', 'position': 'fixed', 'top': coords.fullTop, 'left': coords.fullLeft, 'height': coords.high, 'width': coords.width, 'maxWidth': coords.width }).removeClass('ui-simpledialog-hidden');
			} else {
				self.sdIntContent.css({'position': 'fixed', 'top': coords.winTop, 'left': coords.winLeft}).removeClass('ui-simpledialog-hidden');
			}
		}
	},
	repos: function() {
	...
	open: function() {
		...
		if ( ( o.dialogAllow === true && coords.width < 400 ) || o.dialogForce ) {
			...
			if ( o.fullScreen === true && ( coords.width < 400 || o.fullScreenForce === true ) ) {
				self.sdIntContent.removeClass('ui-simpledialog-container').css({'border': 'none', 'position': 'fixed', 'top': coords.fullTop, 'left': coords.fullLeft, 'height': coords.high, 'width': coords.width, 'maxWidth': coords.width }).removeClass('ui-simpledialog-hidden');
			} else {
				self.sdIntContent.css({'position': 'fixed', 'top': coords.winTop, 'left': coords.winLeft}).removeClass('ui-simpledialog-hidden');
			}
